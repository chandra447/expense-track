...
export const chatRoute = new Hono()
  .post("/", async (c) => {
    ...
    try {
      ...
      // If threadId is provided, verify the thread belongs to the user
      if (threadId) {
...
IMPORTANT: Only specify a date when the user explicitly mentions a specific date. If no date is mentioned, let the system use the current timestamp automatically.`,
        ...
        onFinish: async ({ text }) => {
          // Store the assistant message if threadId is provided
          if (threadId) {
            await db.insert(chatMessages).values({
              id: nanoid(),
              threadId,
  ...
  // New thread-based chat endpoint
  .post("/:threadId", async (c) => {
    const auth = getAuth(c);
